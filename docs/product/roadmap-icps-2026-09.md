...
| D3 | Notificações (avisar quando uma lista aguardada é publicada, quando uma avaliação/sugestão é decidida) | Pais | **Risco de colisão em andamento, não decidido ainda.** Há uma segunda sessão paralela (mesma `session_018dpJRsVgtysxT6o6pWqVTY` da Onda 2) trabalhando agora nos prompts 14-20 (`docs/prompts/`) numa branch ainda não publicada (`feat/onda-4-admin-listas`), e já aplicou direto no Postgres compartilhado duas migrations que nenhuma branch git ainda tem: `admin_publish_list` e `list_notification_requests` — o segundo nome sugere que isso já é este item D3, ou parte dele. Não vou desenhar/implementar D3 até essa branch aparecer no GitHub e eu conferir o que ela cobre de fato — evita construir uma segunda tabela de notificação em paralelo com quem já está fazendo isso. |
