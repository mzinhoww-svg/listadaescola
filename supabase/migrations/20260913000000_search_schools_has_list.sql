-- Onda 2 / P2+P3 (docs/implementation/impeccable-critique.md).
--
-- Problema: o catálogo promete "descubra a lista da sua escola" e a tela de
-- descoberta não sabe dizer quais escolas têm lista. Com 2.722 escolas e
-- cobertura parcial, a única forma de descobrir era abrir o perfil de cada
-- uma. `list_count` já era retornado, mas não dava para filtrar nem ordenar
-- por ele.
--
-- Acrescenta:
--   * p_has_list  -> true: só escolas com lista publicada; false: só as sem;
--                    null (default): sem filtro, comportamento de hoje.
--   * p_sort = 'lists' -> ordena por list_count desc.
--
-- Por que DROP e não só CREATE OR REPLACE: em Postgres, acrescentar um
-- parâmetro (mesmo com default) cria uma SOBRECARGA, não substitui a função.
-- Como todos os parâmetros têm default, as duas assinaturas ficariam
-- ambíguas para qualquer chamada nomeada e o PostgREST erraria. O drop da
-- assinatura antiga é obrigatório, e por isso os grants são refeitos abaixo.

drop function if exists public.search_schools(
  text, numeric, numeric, text, text, text, numeric,
  public.school_type, text, numeric, text, int, int
);
...
comment on function public.search_schools(text, numeric, numeric, text, text, text, numeric, public.school_type, text, numeric, text, int, int, boolean) is
  'Busca de escolas com ranking orgânico + patrocínio. p_has_list filtra por disponibilidade de lista publicada (true/false/null=todas) e p_sort=''lists'' ordena por list_count -- Onda 2 P2/P3: o catálogo precisa saber dizer quais escolas têm lista. SECURITY DEFINER, respeita schools_select_active.';
